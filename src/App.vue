...
const toggleRole = () => {
  role.value = role.value === "Admin" ? "user" : role.value === "user" ? "Admin" : "N/A";
};
...
onMounted(async () => {
  try {
    ...
    const fetchDetails = async (name) => {
      ...
      const res = await fetch(`https://api.resonance.rest/characters/${encodedName}`);
    ...
    characters.value = detailedCharacters.map(c => ({
      name: c.name,
      weapon: c.weapon || "Unknown",
      attribute: c.attribute || "Unknown",
      owned: false,
    }));
  } catch (err) {
...
<template>
  <div class="container">
    <header>
      ...
      <div v-if="role === 'Admin'">
        <form @submit.prevent="editIndex !== null ? updateCharacter() : addCharacter" class="form-section">
    ...
    <section class="character-list">
      ...
      >
        ...
        <button @click="editCharacter(index)" title="Edit character">✎</button>

        <button @click="deleteCharacter(index)" title="Delete character">x</button>
      </div>
    </section>
